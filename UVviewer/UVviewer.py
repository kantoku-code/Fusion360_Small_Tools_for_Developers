    ...
    def notify(self, args :adsk.core.MouseEventArgs):
        try:
            ...
            if prms:
                ...
                verPrms = ['{:.3f}:{:.3f}'.format(p[0], p[1]) for p in prms[1:]]
                _verIpt.text = '\n'.join(verPrms)
            else:
                _prmIpt.text = '-'
                _verIpt.text = '-'
...
class CommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            _selIpt = inputs.addSelectionInput(_selInfo[0], _selInfo[1], _selInfo[2])
            _selIpt.isVisible = False

            global _prmInfo, _prmIpt
            _prmIpt = inputs.addTextBoxCommandInput(_prmInfo[0], _prmInfo[1], _prmInfo[2], 1, True)

            global _verInfo, _verIpt
            _verIpt = inputs.addTextBoxCommandInput(_verInfo[0], _verInfo[1], _verInfo[2], 10, True)
...
class DrawUVFactry():
    ...
    def update(self):
        ...
        # vertex
        vertices = [p.geometry for p in self._clone.vertices]
